-- Create Sifa Achievements System (Safe Version)
...
DROP FUNCTION IF EXISTS get_achievement_with_details(UUID);
DROP FUNCTION IF EXISTS get_all_achievements_with_details(INTEGER, INTEGER);
DROP FUNCTION IF EXISTS get_user_achievements_with_details(UUID, INTEGER, INTEGER);
DROP FUNCTION IF EXISTS get_achievement_media(UUID);
DROP FUNCTION IF EXISTS get_achievement_comments(UUID, INTEGER, INTEGER);
...
BEGIN
  ...
  SELECT 
    ...
    CASE WHEN user_likes.liked_user_id IS NOT NULL THEN true ELSE false END as user_liked
  FROM achievements a
  ...
  LEFT JOIN (
    SELECT achievement_id, user_id as liked_user_id
    FROM achievement_likes
    WHERE achievement_likes.user_id = auth.uid()
