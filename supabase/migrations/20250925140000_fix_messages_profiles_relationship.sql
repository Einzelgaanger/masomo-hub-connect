...
-- This ensures Supabase can properly join messages with profiles

-- Add foreign key constraint from messages.user_id to profiles.user_id
DO $$
BEGIN
    IF NOT EXISTS (
...
-- Add foreign key constraint from message_likes.user_id to profiles.user_id  
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.table_constraints 
...
-- Refresh the schema cache to ensure relationships are detected
